...
func (vcd *TestVCD) TestDiskResizeInFastProvisionedVdc(check *C) {
	org, err := vcd.client.GetAdminOrgByName(vcd.config.VCD.Org)
	check.Assert(err, IsNil)
	check.Assert(org, NotNil)
	...
	//
	emptyvApp, err := vdc.GetVAppByName("empty", true)
	check.Assert(err, IsNil)
	...
	var def = types.InstantiateVAppTemplateParams{
		...
		IsSourceDelete: false,
		VAppParent: &types.Reference{
			HREF: emptyvApp.VApp.HREF,
			ID:   emptyvApp.VApp.ID,
			Name: emptyvApp.VApp.Name,
			Type: emptyvApp.VApp.Type,
		},
		AllEULAsAccepted: true,
	}
