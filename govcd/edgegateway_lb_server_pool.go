...
func (eGW *EdgeGateway) CreateLBServerPool(lbPoolConfig *types.LBPool) (*types.LBPool, error) {
	...
	httpPath, err := eGW.getProxiedEdgeEndpoint("/loadbalancer/config/pools")
	if err != nil {
		return nil, fmt.Errorf("could not get Edge Gateway API endpoint: %s", err)
	}
	// We expect to get http.StatusCreated or if not an error of type types.NSXError
	resp, err := eGW.client.ExecuteRequestHTTPCodeOrTypedError(http.StatusCreated, httpPath, http.MethodPost, "application/xml", "error creating load balancer server pool: %s", lbPoolConfig, &types.NSXError{})
	if err != nil {
		return nil, err
	}
	...
	if location == "" {
		return nil, fmt.Errorf("unable to retrieve ID for new load balancer service monitor with name %s", lbPoolConfig.Name)
	}
...
func (eGW *EdgeGateway) ReadLBServerPool(lbPoolConfig *types.LBPool) (*types.LBPool, error) {
	...
	_, err = eGW.client.ExecuteRequest(httpPath, http.MethodGet, "application/xml", "unable to read Load Balancer pool: %s", nil, lbPoolResponse)
	if err != nil {
		return nil, err
	}
	...
	for _, pool := range lbPoolResponse.LBPools {
		...
		if lbPoolConfig.Name != "" && pool.Name == lbPoolConfig.Name {
			...
			if lbPoolConfig.ID != "" && pool.ID != lbPoolConfig.ID {
				return nil, fmt.Errorf("load balancer pool was found by name (%s), but it's ID (%s) does not match specified ID (%s)",
	...
	return nil, fmt.Errorf("could not find load balancer pool (name: %s, ID: %s)",
...
func (eGW *EdgeGateway) UpdateLBServerPool(lbPoolConfig *types.LBPool) (*types.LBPool, error) {
	...
	httpPath, err := eGW.getProxiedEdgeEndpoint("/loadbalancer/config/pools/" + lbPoolConfig.ID)
	if err != nil {
		return nil, fmt.Errorf("could not get Edge Gateway API endpoint: %s", err)
	}

	// Result should be 204, if not we expect an error of type types.NSXError
	_, err = eGW.client.ExecuteRequestHTTPCodeOrTypedError(http.StatusNoContent, httpPath, http.MethodPut, "application/xml", "%s", lbPoolConfig, &types.NSXError{})
	if err != nil {
		return nil, fmt.Errorf("error while updating load balancer server pool : %s", err)
	}

	readPool, err := eGW.ReadLBServerPool(&types.LBPool{ID: lbPoolConfig.ID})
	if err != nil {
		return nil, fmt.Errorf("unable to retrieve pool with ID (%s) after update: %s", readPool.ID, err)
	}
	return readPool, nil
}
...
func (eGW *EdgeGateway) DeleteLBServerPool(lbPoolConfig *types.LBPool) error {
	...
	httpPath, err := eGW.getProxiedEdgeEndpoint("/loadbalancer/config/pools/" + lbPoolID)
	if err != nil {
